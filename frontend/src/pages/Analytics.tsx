...
export default function Analytics() {
  ...
  const { data, isLoading, isError } = useQuery<Dashboard>({
    queryKey: ["analytics-dashboard", isAdmin ? scope : "self", period],
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <BarChart3 size={22} className="text-brand-500" /> Search Analytics
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">
            Derived from every search — demand, sentiment, coverage gaps and risk
            {data && <> · <span className="font-medium text-slate-600">{data.scope_label}</span></>}
          </p>
        </div>
      ...
      {isLoading && (
        <div className="h-64 flex items-center justify-center text-slate-400">
          <Loader2 className="animate-spin" /> <span className="ml-2 text-sm">Crunching the search log…</span>
        </div>
      )}
      {isError && (
