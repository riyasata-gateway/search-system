...
# A focused queue beats an exhaustive one — pharmacists act on a handful of the
# strongest gaps, not a wall of every missing SKU.
_MAX_RECOMMENDATIONS = 7
_MAX_PER_CATEGORY = 3


async def _category_names(db: AsyncSession, ids) -> dict:
    ids = {i for i in ids if i is not None}
    if not ids:
        return {}
    rows = (await db.execute(
        select(ProductCategory.id, ProductCategory.name_en).where(ProductCategory.id.in_(ids))
    )).all()
    return {r.id: r.name_en for r in rows}


async def _product_names(db: AsyncSession, ids) -> dict:
    ids = {i for i in ids if i is not None}
    if not ids:
        return {}
    rows = (await db.execute(
...
def _diversify(recs, limit=_MAX_RECOMMENDATIONS, per_category=_MAX_PER_CATEGORY):
    """Take the top `limit` recs (already confidence-sorted) but cap how many can
    come from one category, so the queue spans the pharmacist's range rather than
    flooding them with one trending category's whole shelf."""
    picked, per = [], {}
    for r in recs:
        c = r.category_id
        if per.get(c, 0) >= per_category:
            continue
        picked.append(r)
        per[c] = per.get(c, 0) + 1
        if len(picked) >= limit:
...
def _rec_out(r: Recommendation, cat_map: dict, prod_map: dict) -> "RecommendationOut":
    out = RecommendationOut.model_validate(r)
    out.category_name = cat_map.get(r.category_id)
    out.product_name = prod_map.get(r.product_id)
    return out
...
async def pharmacist_dashboard(
    ...
    rec_result = await db.execute(
        ...
        ).order_by(Recommendation.confidence_score.desc()).limit(40)
    )
    recommendations = _diversify(rec_result.scalars().all())
    ...
    cat_map = await _category_names(
        db, {t.entity_id for t in trending} | {r.category_id for r in recommendations}
    )
    prod_map = await _product_names(db, {r.product_id for r in recommendations})
    ...
    return PharmacistDashboardOut(
        ...
        trending_categories=[
            TrendingCategoryOut(
                ...
                category_name=cat_map.get(t.entity_id),
                country=t.country,
                region=t.region,
                city=t.city,
                score=float(t.score),
        ...
        top_recommendations=[_rec_out(r, cat_map, prod_map) for r in recommendations],
    )
...
async def list_recommendations(
    ...
    limit: int = Query(_MAX_RECOMMENDATIONS, ge=1, le=200),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_pharmacist),
):
    if current_user.pharmacy_id is None:
    ...
    # Fetch a pool, then diversify across categories down to `limit` so the queue
    # stays a focused, range-spanning top-N rather than one category's whole shelf.
    q = q.order_by(Recommendation.confidence_score.desc()).offset(skip).limit(max(limit * 6, 40))
    result = await db.execute(q)
    recs = _diversify(result.scalars().all(), limit=limit)

    cat_map = await _category_names(db, {r.category_id for r in recs})
    prod_map = await _product_names(db, {r.product_id for r in recs})
    return [_rec_out(r, cat_map, prod_map) for r in recs]
...
async def trending_categories(
    ...
    cat_map = await _category_names(db, {r.entity_id for r in rows})
    return [
        TrendingCategoryOut(
            entity_type=r.entity_type,
            entity_id=r.entity_id,
            category_name=cat_map.get(r.entity_id),
            country=r.country,
            region=r.region,
            city=r.city,
            score=float(r.score),
